            ...
            if (service != nullptr) {
                ...
                if (name != nullptr) {
                    std::string sensor_name =
                        SafeCFStringToStdString(name).value_or("");
                    CFRelease(name);
...
GPUActivities::GPUActivities(io_object_t entry) {
    ...
    auto creator =
        SafeCFStringToStdString(static_cast<CFStringRef>(creatorRef));
    ...
    for (CFIndex i = 0; i < count; ++i) {
        ...
        if (usageStats) {
            ...
            CFDictionaryGetKeysAndValues(usageStats, keys.data(),
                                         values.data());
            ...
            for (CFIndex i = 0; i < count; ++i) {
                ...
                auto number =
                    SafeIOServiceGetNumberFromDictionary(usageStats, keyRef);
                if (number.has_value()) {
                ...
                auto string =
                    SafeIOServiceGetStringFromDictionary(usageStats, keyRef);
                if (string.has_value()) {
...
bool GPU::childrenIteratorCallback(io_object_t object, void *data) {
    ...
    self->actual_activities.insert_or_assign(
        newActivity.proc,
        std::make_tuple(std::move(newActivity), totalUsage, 0));
...
void GPU::Lookup(io_object_t ioAccelerator) {
    ...
    if (perfStats) {
        ...
        for (CFIndex i = 0; i < count; ++i) {
            auto key =
                SafeCFStringToStdString(static_cast<CFStringRef>(keys[i]));
            if (!key)
                continue;

            auto value =
                SafeCFNumberToInt64(static_cast<CFNumberRef>(values[i]));
            if (key)
...
GPU::GPU(io_object_t ioAccelerator) {
    ...
    IOServiceGenericIterator("AppleARMIODevice",
                             appleArmIoDeviceIteratorCallback, this);
